...
public class TestNGramMap {
    ...
    public void testCountHistory() {
        ...
        for (int i = 0; i < expectedCounts.size(); i += 1) {
            System.out.println(request2005to2008.data().get(i));
    ...
    public void testOnSmallFile() {
        ...
        TimeSeries wanderedCount = ngm.countHistory("wandered", 2005, 2007);
        assertEquals(83769.0, wanderedCount.get(2005), 1E-10);
        assertEquals(108634.0, wanderedCount.get(2007), 1E-10);

        TimeSeries totalCounts = ngm.totalCountHistory();
        assertEquals(6.0, totalCounts.get(1484), 1E-10);

        TimeSeries wanderedWeight = ngm.weightHistory("wandered", 2005, 2007);
        assertEquals(83769.0/26609986084.0, wanderedWeight.get(2005), 1E-7);

        TimeSeries requestCount = ngm.countHistory("request", 2005, 2007);
        assertEquals(646179.0, requestCount.get(2005), 1E-10);

        List<String> wanderedAndRequest = new ArrayList<>();
        wanderedAndRequest.add("wandered");
        wanderedAndRequest.add("request");
        TimeSeries wanderedPlusRequestWeight = ngm.summedWeightHistory(wanderedAndRequest, 2005, 2007);
        double expectedWanderedPlusRequestWeight2005 = (83769.0 + 646179.0) / 26609986084.0;
        assertEquals(expectedWanderedPlusRequestWeight2005, wanderedPlusRequestWeight.get(2005), 1E-10);
    }
